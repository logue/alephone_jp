...
Loren Petrich, 4-8-00:
Added typecasting to mallocs, so as to make CodeWarrior happy
*/
				...
				BINDING **bindingsPtr)
{
  ...
  do {
    if (i == binding->uriAlloc) {
      binding->uri = (char *)realloc(binding->uri, binding->uriAlloc *= 2);
      if (!binding->uri)
	return XML_ERROR_NO_MEMORY;
    }
...
int addBinding(XML_Parser parser, PREFIX *prefix, const ATTRIBUTE_ID *attId, const XML_Char *uri, BINDING **bindingsPtr)
{
  ...
  if (freeBindingList) {
    b = freeBindingList;
    if (len > b->uriAlloc) {
      b->uri = (char *)realloc(b->uri, len + EXPAND_SPARE);
      if (!b->uri)
	return 0;
...
/* 自動生成したJIS<->UNICODE変換テーブル */
#include "jutable.c"

/**
* 1文字のJIS208をUNICODEに変換する。
 * @param ch   JIS208の文字コード。
...
/**
* UNICODEに対応するJIS208のコードを検索する。
 * @param ch    UNICODEの文字コード。
 * @param start 検索レコードの先頭。
 * @param end   検索レコードの終端。
 * @return 対応するJIS208の文字コード。対応するものがなければ0。
 */
...
/**
* UNICODEをJIS201カナに変換する。
 * @param ch UNICODEの文字コード。
 * @return 対応するJIS201カナの文字コード。対応するものがなければ0。
 */
...
/**
* EUC-JPの文字列から一文字取りだしUNICODEに変換する。
 * @param s    EUC-JPの文字列。
 * @param next 次のスキャン位置を格納するアドレス。NULLが指定された場合は格納しない。
 * @return 対応するUNICODEの文字コード。対応するものがなければ0。
 */
...
/**
* SHIFT_JISの文字列から一文字取りだしUNICODEに変換する。
...
/**
* UTF8の文字列から一文字取りだしEUC-JPに変換する。
 * @param p    UTF8の文字列。
 * @param next 次のスキャン位置を格納するアドレス。NULLが指定された場合は格納しない。
 * @return 対応するEUC-JPの文字コード。対応するものがなければ0。
 */
...
/**
* UTF8の文字列から一文字取りだしSHIFT_JISに変換する。
 * @param p    UTF8の文字列
 * @param next 次のスキャン位置を格納するアドレス。NULLが指定された場合は格納しない。
 * @return 対応するSHIFT_JISの文字コード。対応するものがなければ0。
 */
