...
boost::unordered_map<std::string, tex_cache> FontSpecifier::texTable;
...
void FontSpecifier::OGL_Render(const char *Text)
{
	...
	if( texTable.find(Text) != texTable.end() ) {
		...
		if( texTable.size() == 256 ) {
			for(boost::unordered_map<std::string, tex_cache>::iterator i =  texTable.begin(); i != texTable.end(); i++) {
