...
Typecode FileSpecifier::GetType()
{
	...
	if (extension) {
		if (strcasecmp((const char*)extension, ".sce2") == 0 ||
            strcasecmp((const char*)extension, ".sceA") == 0) return _typecode_scenario;
		
		// don't bother checking files with these extensions
		// this helps a lot because there can be many of these files
		if (
			strcasecmp((const char*)extension, ".dds") == 0 ||
			strcasecmp((const char*)extension, ".jpg") == 0 ||
			strcasecmp((const char*)extension, ".png") == 0
			) 
		{
