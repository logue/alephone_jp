...
static inline int draw_text(SDL_Surface *s, const char *text, size_t length, int x, int y, uint32 pixel, const font_info *font, uint16 style, bool utf8 = true)
{
	return font ? font->draw_text(s, text, length, x, y, pixel, style, utf8) : 0;
}

static inline int draw_text(SDL_Surface *s, const char *text, int x, int y, uint32 pixel, const font_info *font, uint16 style, bool utf8 = true)
{
